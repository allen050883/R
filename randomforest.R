...
library(compiler)
enableJIT(3)
FOREST_model <- randomForest(Demanda_uni_equil~Canal_ID+Ruta_SAK+Cliente_ID , data=training, mtry=3, ntree=500, importance=TRUE, do.trace=100)
print(FOREST_model)
result_1=predict(FOREST_model, newdata = test)
result_1
write.csv(result_1,file="result_randomforest_Client_ID_1.csv")
